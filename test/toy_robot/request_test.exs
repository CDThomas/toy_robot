defmodule ToyRobot.RequestTest do
  ...
  describe "get/1" do
    ...
    test "returns the response data as a map" do
      with_mock HTTPoison, get: fn _ -> @get_response end do
        assert Request.get("/report") == {:ok, %{"direction" => "west", "x" => 0, "y" => 1}}
      end
    end

    test "returns an error if the request fails" do
      with_mock HTTPoison, get: fn _ -> @error_response end do
        assert Request.get("/report") == @error_response
      end
    end
  end
  ...
  describe "post/2" do
    ...
    test "returns the response data as a map" do
      with_mock HTTPoison, post: fn _, _, _ -> @post_response end do
        ...
        assert resp == {:ok, %{"direction" => "north", "x" => 0, "y" => 1}}
      end
    end

    test "returns an error if the request fails" do
      with_mock HTTPoison, post: fn _, _, _ -> @error_response end do
        assert Request.post("/place", %{x: 0, y: 0, direction: :north}) == @error_response
      end
    end
  end
